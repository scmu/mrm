...
mlift' :: Monad m => TList fs -> Matches fs a b -> Matches fs (m a) (m b)
mlift' TNil       Void       = Void
mlift' (TCons ts) (k ::: ks) = (liftM k . sequence) ::: mlift' ts ks

mlift :: (Monad m, Traversables fs) => Matches fs a b -> Matches fs (m a) (m b)
mlift = mlift' trep
