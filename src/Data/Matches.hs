...
generateMatches' :: (forall f. Functor f => Elem f fs -> f a -> b) -> Sub gs fs -> Matches gs a b
generateMatches' f SNil = Void
generateMatches' f (SCons pos ss) = f pos ::: generateMatches' f ss

generateMatches :: (fs <: fs) => (forall f. Functor f => Elem f fs -> f a -> b) -> Matches fs a b
generateMatches f = generateMatches' f srep

constMatches :: (fs <: fs) => b -> Matches fs a b
constMatches y = generateMatches (\_ _ -> y)

generateMatches2 :: (forall f. Functor f => f a -> b) -> FList fs -> Matches fs a b
...
class Union fs gs hs | fs gs -> hs where
    union :: Matches fs a b -> Matches gs a b -> Matches hs a b
