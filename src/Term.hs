...
renameAlg :: (fs <: gs, Mem Var fs, Mem Var gs) => Algebras fs (Fix gs)
renameAlg = (\(Var s) -> var (s ++ "_")) >:: transAlg

rename :: (fs <: fs, Mem Var fs) => Fix fs -> Fix fs
rename = fold renameAlg

desugarAlg :: (fs <: gs, Mem App gs, Mem Lam gs) => Algebras (Let ': fs) (Fix gs)
desugarAlg =
    (\(Let x e1 e2) -> app (lam x e2) e1) :::
    transAlg
...
freeVars :: (Foldables fs, Mem Lam fs, Mem Var fs, Mem Let fs) => Fix fs -> [VName]
